...
public:
    ...
    void setBoundingBox(sf::FloatRect boundingBox) { this->boundingBox = boundingBox; }
    ...
    void applyResistance(float resistance) {
		if(movement == sf::Vector2f{0,0}) return;
    ...
    void cap(float terminal){
        if( movement.lengthSquared() <= (terminal*terminal) ) return;
