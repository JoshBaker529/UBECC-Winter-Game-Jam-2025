...
public:
  ...
  MainState():
	...
	background( sf::Vector2i(100,100), sf::Vector2f(32.f, 32.f), sf::Vector2f(96.f, 96.f), sf::Vector2f(-32.f,-32.f) ),
	shadows( sf::Vector2i(100,100), sf::Vector2f(32.f, 32.f), sf::Vector2f(96.f, 96.f), sf::Vector2f(-32.f,-32.f) ),
	...
	windAngle(sf::degrees(0.f))
	{
		...
		if(!frontTileset.loadFromFile("res/frontTiles.png")) exit(1);
		if(!frontTileset.generateMipmap()) exit(1);
		
		if(!backTileset.loadFromFile("res/backTiles.png")) exit(1);
		if(!backTileset.generateMipmap()) exit(1);
		
		if(!characterTexture.loadFromFile("res/characters.png")) exit(1);
		if(!characterTexture.generateMipmap()) exit(1);
		
		if(!itemTexture.loadFromFile("res/items.png")) exit(1);
		if(!itemTexture.generateMipmap()) exit(1);
		
		if(!shadowTexture.loadFromFile("res/shadow.png")) exit(1);
		if(!shadowTexture.generateMipmap()) exit(1);
		
		if(!fogTexture.loadFromFile("res/fog.png")) exit(1);
		
		
		nightRect.setFillColor(sf::Color::Blue);
		
		if(!font.openFromFile("res/tuffy.ttf")) exit(1);
